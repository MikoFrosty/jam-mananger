...
export function fetchClients() {
  ...
  return async function(dispatch) {
    try {
      const { clients, client_invitations } = await fetchWrapper("/client", localStorage.getItem("token"), "GET")
        .then((res) => {
          return { clients: res.clients, client_invitations: res.client_invitations };
        });
...
export function fetchDocuments() {
  return async function(dispatch) {
    try {
      const response = await fetchWrapper("/documents", localStorage.getItem("token"), "GET");
...
export function fetchFolders() {
  return async function(dispatch) {
    try {
      const response = await fetchWrapper("/folders", localStorage.getItem("token"), "GET");
      // Assuming the response from fetchWrapper is the actual data you want to set
      console.log(response)
      dispatch({
        type: "SET_FOLDERS",
